...
function shop_stop($bot_dir,$admin_dir) {
	exec("forever stop -p $bot_dir ${bot_dir}bot_shop.js", $out);
	return false;
}
...
else if($URIa[3]=='shop_action') {
	// SET STATUS	
	if($URIa[4]=='start_shop') {
		set_time_limit(4);
		if(!shop_status()) {
			shop_start($bot_dir,$admin_dir); 
			echo 'starting';
		}
		else {
			shop_stop($bot_dir,$admin_dir);
			shop_start($bot_dir,$admin_dir); 
			echo 'running';
		}
	}
	else if($URIa[4]=='stop_shop') {
		shop_stop($bot_dir,$admin_dir);
		echo 'stopping';
	}
	else if($URIa[4]=='restart_shop') {
		if(!shop_status()) {
			shop_start($bot_dir,$admin_dir); 
			echo 'restart';
		}
		else {
			if(shop_stop($bot_dir,$admin_dir)) 
			shop_start($bot_dir,$admin_dir);
			echo 'restart';
		}
	}
